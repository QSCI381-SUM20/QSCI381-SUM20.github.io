...
# load data
daily_rain <-
  readr::read_csv("../../static/data/seattleWeather_1948-2017.csv") %>%
...
d. **Assume that the number of days of rain in July in Seattle is a binomial random variable, with *p*, the probability of rain on a single day, given by your answer to b. What is the theoretical probability of observing more than seven days of rain in July in Seattle?**
...
f. **As in d., assume that the number of days of rain in July in Seattle is a binomial random variable, with *p*, the probability of rain on a single day, given by your answer to b. Plot the theoretical binomial distribution.**

    **YOUR ANSWER HERE**
    
g. **Is it appropriate to model the number of days of rain in July as a binomial random variable? Why or why not? If not, what other distributions could we use?**

    **YOUR ANSWER HERE**
    
h. **Now, for each month, give an estimate of the probability that it will rain on any single day in Seattle. Provide a one-sentence interpretation of your results.**

    **YOUR ANSWER HERE**
    
i. **Generate a visualization of your choice that provides some insight about this dataset. Provide a one-sentence summary of your visualization. Full points will be given for clarity and creativity.**
...
a. **`X` was sampled from either a normal, binomial, geometric, or Poisson distribution. Which distribution seems like the most appropriate model for `X`? Justify your answer and give an estimate of any model parameters.**

    **YOUR ANSWER HERE**
    
b. **`Y` was sampled from either a normal, binomial, geometric, or Poisson distribution. Which distribution seems like the most appropriate model for `Y`? Justify your answer and give an estimate of any model parameters.**
...
**Suppose SAT scores closely follow the normal model with mean $\mu=1100$ and standard deviation $\sigma = 200$.**
